...
def main(args):
    ...
    os.makedirs(os.path.join(edit_directory_path,'coupled'), exist_ok=True)
    ...
    for i, batch in enumerate(data_loader):
        ...
        if args.edit_attribute == 'inversion':
            ...
            img_edit, edit_latents = editor.apply_interfacegan(latent_codes[i].unsqueeze(0).to(device), edit_direction,
            ...
            img_edit, edit_latents = editor.apply_ganspace(latent_codes[i].unsqueeze(0).to(device), ganspace_pca,
                                                           [edit_direction])
        ...
        if  is_cars:
            x_img=tensor2im(x[0])
            imgs_resize = torch.nn.functional.interpolate(torch.clamp(imgs, -1., 1.), size=(192, 256), mode='bilinear')
            result = tensor2im(imgs_resize[0])
            couple = np.concatenate([np.array(x_img), np.array(result)], axis=1)
            Image.fromarray(couple).save(os.path.join(edit_directory_path, 'coupled', f"{i:05d}.jpg"))
